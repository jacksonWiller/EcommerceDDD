    ...
    public class RepositoryProduct : RepositoryGenerics<Produto>, IProduct
    {
        ...
        public async Task<List<Produto>> ListarProdutosCarrinhoUsuario(string userId)
        {
            using (var banco = new ContextBase(_optionsbuilder))
            {
                ...
                var param = new SqlParameter("@CUS_ESTADO", (int)EnumEstadoCompra.Produto_Carrinho);
                var param2 = new SqlParameter("@UserId", userId);

                var produtosCarrinhoUsuario = 
                    await banco.Produto.FromSqlRaw("SP_PRODUTOS_CARRINHO_USUARIO @CUS_ESTADO,@UserId", param, param2).ToListAsync();
                ...
                //                {
                //                  new SqlParameter("@nome", SqlDbType.VarChar, 250) { Value = prod.Descricao },
                //                  new SqlParameter("@valor", SqlDbType.Decimal) { Value = prod.Valor },

                //                };

                //await banco.Database.ExecuteSqlRawAsync("SP_PRODUTOS_CARRINHO_USUARIO", parameters);
